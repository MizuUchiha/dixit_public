<?php


class Conn
{
    public $conn;
    private $stmt;

    public function __construct()
    {
        $this->conn = new mysqli($_ENV['HOST'], $_ENV['USER'], $_ENV['PASS'], $_ENV['DB_NAME']);

        if ($this->conn->connect_error) {
            die("Connection failed: " . $this->conn->connect_error);
        }
    }

    public function stmt($stmt)
    {
        if ($this->stmt = $this->conn->prepare($stmt)) {
            if (func_num_args() > 1) {
                $x = func_get_args();
                $args = array_slice($x, 1);
                $types = '';
                $args_ref = array();
                foreach ($args as $k => &$arg) {
                    if (is_array($args[$k])) {
                        foreach ($args[$k] as $j => &$a) {
                            $types .= $this->_gettype($args[$k][$j]);
                            $args_ref[] = &$a;
                        }
                    } else {
                        $types .= $this->_gettype($args[$k]);
                        $args_ref[] = &$arg;
                    }
                }
                array_unshift($args_ref, $types);
                call_user_func_array(array($this->stmt, 'bind_param'), $args_ref);
            }
            $this->stmt->execute();
            if ($this->stmt->errno) {
                die('Unable to process MySQL stmt (check your params) - ' . $this->stmt->error);
            }
        } else {
            die('Unable to prepare statement (check your syntax) - ' . $this->conn->error);
        }
        return $this;
    }

    public function fetchAll()
    {
        $params = array();
        $meta = $this->stmt->result_metadata();
        while ($field = $meta->fetch_field()) {
            $params[] = &$row[$field->name];
        }
        call_user_func_array(array($this->stmt, 'bind_result'), $params);
        $result = array();
        while ($this->stmt->fetch()) {
            $r = array();
            foreach ($row as $key => $val) {
                $r[$key] = $val;
            }
            $result[] = $r;
        }
        $this->stmt->close();
        return $result;
    }

    public function close()
    {
        return $this->conn->close();
    }

    /*
     * GET VARIABLE TYPE
     */
    private function _gettype($var)
    {
        if (is_string($var)) return 's';
        if (is_float($var)) return 'd';
        if (is_double($var)) return 'd';
        if (is_int($var)) return 'i';
        return 'b';
    }

}